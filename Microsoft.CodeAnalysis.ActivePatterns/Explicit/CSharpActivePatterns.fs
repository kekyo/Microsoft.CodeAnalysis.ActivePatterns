...
module ActivePatterns =
  ...
  let (|IdentifierNameSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.IdentifierNameSyntax as node ->
      Some (node.Identifier)
    | _ -> None

  let (|QualifiedNameSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.QualifiedNameSyntax as node ->
      Some (node.Left, node.DotToken, node.Right)
    | _ -> None

  let (|GenericNameSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|TupleExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.TupleExpressionSyntax as node ->
      Some (node.OpenParenToken, node.CloseParenToken)
    | _ -> None

  let (|PrefixUnaryExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.PrefixUnaryExpressionSyntax as node ->
      Some (node.OperatorToken, node.Operand)
    | _ -> None

  let (|AwaitExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.AwaitExpressionSyntax as node ->
      Some (node.AwaitKeyword, node.Expression)
    | _ -> None

  let (|PostfixUnaryExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.PostfixUnaryExpressionSyntax as node ->
      Some (node.Operand, node.OperatorToken)
    | _ -> None

  let (|MemberAccessExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ConditionalAccessExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|MemberBindingExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.MemberBindingExpressionSyntax as node ->
      Some (node.OperatorToken, node.Name)
    | _ -> None

  let (|ElementBindingExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ElementBindingExpressionSyntax as node ->
      Some (node.ArgumentList)
    | _ -> None

  let (|ImplicitElementAccessSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ImplicitElementAccessSyntax as node ->
      Some (node.ArgumentList)
    | _ -> None

  let (|BinaryExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.BinaryExpressionSyntax as node ->
      Some (node.Left, node.OperatorToken, node.Right)
    | _ -> None

  let (|AssignmentExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.AssignmentExpressionSyntax as node ->
      Some (node.Left, node.OperatorToken, node.Right)
    | _ -> None

  let (|ConditionalExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ThisExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ThisExpressionSyntax as node ->
      Some (node.Token)
    | _ -> None

  let (|BaseExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.BaseExpressionSyntax as node ->
      Some (node.Token)
    | _ -> None

  let (|LiteralExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.LiteralExpressionSyntax as node ->
      Some (node.Token)
    | _ -> None

  let (|MakeRefExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|RefTypeExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.RefTypeExpressionSyntax as node ->
      Some (node.Keyword, node.OpenParenToken, node.Expression, node.CloseParenToken)
    | _ -> None

  let (|RefValueExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|CheckedExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.CheckedExpressionSyntax as node ->
      Some (node.Keyword, node.OpenParenToken, node.Expression, node.CloseParenToken)
    | _ -> None

  let (|DefaultExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|TypeOfExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.TypeOfExpressionSyntax as node ->
      Some (node.Keyword, node.OpenParenToken, node.Type, node.CloseParenToken)
    | _ -> None

  let (|SizeOfExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.SizeOfExpressionSyntax as node ->
      Some (node.Keyword, node.OpenParenToken, node.Type, node.CloseParenToken)
    | _ -> None

  let (|InvocationExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.InvocationExpressionSyntax as node ->
      Some (node.Expression, node.ArgumentList)
    | _ -> None

  let (|ElementAccessExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ElementAccessExpressionSyntax as node ->
      Some (node.Expression, node.ArgumentList)
    | _ -> None

  let (|ArgumentListSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ArgumentListSyntax as node ->
      Some (node.OpenParenToken, node.CloseParenToken)
    | _ -> None

  let (|BracketedArgumentListSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ArgumentSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|NameColonSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.NameColonSyntax as node ->
      Some (node.Name, node.ColonToken)
    | _ -> None

  let (|DeclarationExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.DeclarationExpressionSyntax as node ->
      Some (node.Type, node.Designation)
    | _ -> None

  let (|CastExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|AnonymousMethodExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|SimpleLambdaExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|RefExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.RefExpressionSyntax as node ->
      Some (node.RefKeyword, node.Expression)
    | _ -> None

  let (|ParenthesizedLambdaExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|InitializerExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.InitializerExpressionSyntax as node ->
      Some (node.OpenBraceToken, node.CloseBraceToken)
    | _ -> None

  let (|ObjectCreationExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|AnonymousObjectMemberDeclaratorSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.AnonymousObjectMemberDeclaratorSyntax as node ->
      Some (node.NameEquals, node.Expression)
    | _ -> None

  let (|AnonymousObjectCreationExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ArrayCreationExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ImplicitArrayCreationExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|StackAllocArrayCreationExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ImplicitStackAllocArrayCreationExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|QueryExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.QueryExpressionSyntax as node ->
      Some (node.FromClause, node.Body)
    | _ -> None

  let (|QueryBodySyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|FromClauseSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|LetClauseSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|JoinClauseSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|QueryContinuationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.QueryContinuationSyntax as node ->
      Some (node.IntoKeyword, node.Identifier, node.Body)
    | _ -> None

  let (|OmittedArraySizeExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.OmittedArraySizeExpressionSyntax as node ->
      Some (node.OmittedArraySizeExpressionToken)
    | _ -> None

  let (|InterpolatedStringExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|IsPatternExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.IsPatternExpressionSyntax as node ->
      Some (node.Expression, node.IsKeyword, node.Pattern)
    | _ -> None

  let (|ThrowExpressionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ThrowExpressionSyntax as node ->
      Some (node.ThrowKeyword, node.Expression)
    | _ -> None

  let (|WhenClauseSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.WhenClauseSyntax as node ->
      Some (node.WhenKeyword, node.Condition)
    | _ -> None

  let (|DeclarationPatternSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.DeclarationPatternSyntax as node ->
      Some (node.Type, node.Designation)
    | _ -> None

  let (|ConstantPatternSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ConstantPatternSyntax as node ->
      Some (node.Expression)
    | _ -> None

  let (|InterpolatedStringTextSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.InterpolatedStringTextSyntax as node ->
      Some (node.TextToken)
    | _ -> None

  let (|InterpolationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|InterpolationAlignmentClauseSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.InterpolationAlignmentClauseSyntax as node ->
      Some (node.CommaToken, node.Value)
    | _ -> None

  let (|InterpolationFormatClauseSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.InterpolationFormatClauseSyntax as node ->
      Some (node.ColonToken, node.FormatStringToken)
    | _ -> None

  let (|GlobalStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.GlobalStatementSyntax as node ->
      Some (node.Statement)
    | _ -> None

  let (|BlockSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|LocalFunctionStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|LocalDeclarationStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|VariableDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.VariableDeclarationSyntax as node ->
      Some (node.Type)
    | _ -> None

  let (|VariableDeclaratorSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|EqualsValueClauseSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.EqualsValueClauseSyntax as node ->
      Some (node.EqualsToken, node.Value)
    | _ -> None

  let (|SingleVariableDesignationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.SingleVariableDesignationSyntax as node ->
      Some (node.Identifier)
    | _ -> None

  let (|DiscardDesignationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.DiscardDesignationSyntax as node ->
      Some (node.UnderscoreToken)
    | _ -> None

  let (|ParenthesizedVariableDesignationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ParenthesizedVariableDesignationSyntax as node ->
      Some (node.OpenParenToken, node.CloseParenToken)
    | _ -> None

  let (|ExpressionStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|EmptyStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.EmptyStatementSyntax as node ->
      Some (node.SemicolonToken)
    | _ -> None

  let (|LabeledStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.LabeledStatementSyntax as node ->
      Some (node.Identifier, node.ColonToken, node.Statement)
    | _ -> None

  let (|GotoStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|BreakStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.BreakStatementSyntax as node ->
      Some (node.BreakKeyword, node.SemicolonToken)
    | _ -> None

  let (|ContinueStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ContinueStatementSyntax as node ->
      Some (node.ContinueKeyword, node.SemicolonToken)
    | _ -> None

  let (|ReturnStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ThrowStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|YieldStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|WhileStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|DoStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ForStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ForEachStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ForEachVariableStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|UsingStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|FixedStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|CheckedStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.CheckedStatementSyntax as node ->
      Some (node.Keyword, node.Block)
    | _ -> None

  let (|UnsafeStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.UnsafeStatementSyntax as node ->
      Some (node.UnsafeKeyword, node.Block)
    | _ -> None

  let (|LockStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|IfStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ElseClauseSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ElseClauseSyntax as node ->
      Some (node.ElseKeyword, node.Statement)
    | _ -> None

  let (|SwitchStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|SwitchSectionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.SwitchSectionSyntax as node ->
      Some (node.Labels |> Seq.toList, node.Statements |> Seq.toList)
    | _ -> None

  let (|CasePatternSwitchLabelSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|CaseSwitchLabelSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.CaseSwitchLabelSyntax as node ->
      Some (node.Keyword, node.Value, node.ColonToken)
    | _ -> None

  let (|DefaultSwitchLabelSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.DefaultSwitchLabelSyntax as node ->
      Some (node.Keyword, node.ColonToken)
    | _ -> None

  let (|TryStatementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|CatchClauseSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|CatchDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|CatchFilterClauseSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|FinallyClauseSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.FinallyClauseSyntax as node ->
      Some (node.FinallyKeyword, node.Block)
    | _ -> None

  let (|CompilationUnitSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ExternAliasDirectiveSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|UsingDirectiveSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|NamespaceDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|AttributeListSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|AttributeTargetSpecifierSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.AttributeTargetSpecifierSyntax as node ->
      Some (node.Identifier, node.ColonToken)
    | _ -> None

  let (|AttributeSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.AttributeSyntax as node ->
      Some (node.Name, node.ArgumentList)
    | _ -> None

  let (|AttributeArgumentListSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.AttributeArgumentListSyntax as node ->
      Some (node.OpenParenToken, node.CloseParenToken)
    | _ -> None

  let (|AttributeArgumentSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.AttributeArgumentSyntax as node ->
      Some (node.NameEquals, node.NameColon, node.Expression)
    | _ -> None

  let (|NameEqualsSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.NameEqualsSyntax as node ->
      Some (node.Name, node.EqualsToken)
    | _ -> None

  let (|TypeParameterListSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.TypeParameterListSyntax as node ->
      Some (node.LessThanToken, node.GreaterThanToken)
    | _ -> None

  let (|TypeParameterSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ClassDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|StructDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.StructDeclarationSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.Keyword, node.Identifier, node.TypeParameterList, node.BaseList, node.ConstraintClauses |> Seq.toList, node.OpenBraceToken, node.Members |> Seq.toList, node.CloseBraceToken, node.SemicolonToken)
    | _ -> None

  let (|InterfaceDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.InterfaceDeclarationSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.Keyword, node.Identifier, node.TypeParameterList, node.BaseList, node.ConstraintClauses |> Seq.toList, node.OpenBraceToken, node.Members |> Seq.toList, node.CloseBraceToken, node.SemicolonToken)
    | _ -> None

  let (|EnumDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|DelegateDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|EnumMemberDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|BaseListSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.BaseListSyntax as node ->
      Some (node.ColonToken)
    | _ -> None

  let (|SimpleBaseTypeSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.SimpleBaseTypeSyntax as node ->
      Some (node.Type)
    | _ -> None

  let (|TypeParameterConstraintClauseSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ConstructorConstraintSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ClassOrStructConstraintSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ClassOrStructConstraintSyntax as node ->
      Some (node.ClassOrStructKeyword)
    | _ -> None

  let (|TypeConstraintSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.TypeConstraintSyntax as node ->
      Some (node.Type)
    | _ -> None

  let (|FieldDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|EventFieldDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ExplicitInterfaceSpecifierSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ExplicitInterfaceSpecifierSyntax as node ->
      Some (node.Name, node.DotToken)
    | _ -> None

  let (|MethodDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|OperatorDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ConversionOperatorDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ConstructorDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ConstructorInitializerSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|DestructorDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|PropertyDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ArrowExpressionClauseSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ArrowExpressionClauseSyntax as node ->
      Some (node.ArrowToken, node.Expression)
    | _ -> None

  let (|EventDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|IndexerDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|AccessorListSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|AccessorDeclarationSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ParameterListSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ParameterListSyntax as node ->
      Some (node.OpenParenToken, node.CloseParenToken)
    | _ -> None

  let (|BracketedParameterListSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.BracketedParameterListSyntax as node ->
      Some (node.OpenBracketToken, node.CloseBracketToken)
    | _ -> None

  let (|ParameterSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|IncompleteMemberSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.IncompleteMemberSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.Type)
    | _ -> None

  let (|DocumentationCommentTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|TypeCrefSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.TypeCrefSyntax as node ->
      Some (node.Type)
    | _ -> None

  let (|QualifiedCrefSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.QualifiedCrefSyntax as node ->
      Some (node.Container, node.DotToken, node.Member)
    | _ -> None

  let (|NameMemberCrefSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.NameMemberCrefSyntax as node ->
      Some (node.Name, node.Parameters)
    | _ -> None

  let (|IndexerMemberCrefSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.IndexerMemberCrefSyntax as node ->
      Some (node.ThisKeyword, node.Parameters)
    | _ -> None

  let (|OperatorMemberCrefSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ConversionOperatorMemberCrefSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|CrefParameterListSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.CrefParameterListSyntax as node ->
      Some (node.OpenParenToken, node.CloseParenToken)
    | _ -> None

  let (|CrefBracketedParameterListSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.CrefBracketedParameterListSyntax as node ->
      Some (node.OpenBracketToken, node.CloseBracketToken)
    | _ -> None

  let (|CrefParameterSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.CrefParameterSyntax as node ->
      Some (node.RefKindKeyword, node.Type, node.RefOrOutKeyword)
    | _ -> None

  let (|XmlElementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.XmlElementSyntax as node ->
      Some (node.StartTag, node.Content |> Seq.toList, node.EndTag)
    | _ -> None

  let (|XmlElementStartTagSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|XmlElementEndTagSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|XmlEmptyElementSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|XmlNameSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.XmlNameSyntax as node ->
      Some (node.Prefix, node.LocalName)
    | _ -> None

  let (|XmlPrefixSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.XmlPrefixSyntax as node ->
      Some (node.Prefix, node.ColonToken)
    | _ -> None

  let (|XmlTextAttributeSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|XmlCrefAttributeSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|XmlNameAttributeSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|XmlCDataSectionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.XmlCDataSectionSyntax as node ->
      Some (node.StartCDataToken, node.EndCDataToken)
    | _ -> None

  let (|XmlProcessingInstructionSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|XmlCommentSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|IfDirectiveTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ElifDirectiveTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ElseDirectiveTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|EndIfDirectiveTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|RegionDirectiveTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|EndRegionDirectiveTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ErrorDirectiveTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|WarningDirectiveTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|BadDirectiveTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|DefineDirectiveTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|UndefDirectiveTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|LineDirectiveTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|PragmaWarningDirectiveTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|PragmaChecksumDirectiveTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ReferenceDirectiveTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|LoadDirectiveTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
  ...
  let (|ShebangDirectiveTriviaSyntax|_|) (node:Microsoft.CodeAnalysis.CSharp.CSharpSyntaxNode) =
    match node with
