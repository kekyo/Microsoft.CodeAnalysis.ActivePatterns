...
module ActivePatterns =
  ...
  let (|AnonymousMethodExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.AnonymousMethodExpressionSyntax as node ->
      Some (node.AsyncKeyword, node.DelegateKeyword, node.ParameterList, node.Block, node.ExpressionBody)
    | _ -> None

  let (|Argument|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ArgumentSyntax as node ->
      Some (node.RefOrOutKeyword, node.NameColon, node.RefKindKeyword, node.Expression)
    | _ -> None

  let (|CSharpArrayRankSpecifier|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicArrayRankSpecifier|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|CSharpAttribute|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.AttributeSyntax as node ->
      Some (node.Name, node.ArgumentList)
    | _ -> None

  let (|VisualBasicAttribute|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|Block|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.BlockSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.OpenBraceToken, node.Statements |> Seq.toList, node.CloseBraceToken)
    | _ -> None
  ...
  let (|BreakStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.BreakStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.BreakKeyword, node.SemicolonToken)
    | _ -> None
  ...
  let (|CheckedStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.CheckedStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.Keyword, node.Block)
    | _ -> None
  ...
  let (|ClassOrStructConstraint|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ClassOrStructConstraintSyntax as node ->
      Some (node.ClassOrStructKeyword, node.QuestionToken)
    | _ -> None
  ...
  let (|CompilationUnit|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.CompilationUnitSyntax as node ->
      Some (node.Externs |> Seq.cast<Microsoft.CodeAnalysis.SyntaxNode> |> Seq.toList, node.Usings |> Seq.cast<Microsoft.CodeAnalysis.SyntaxNode> |> Seq.toList, node.AttributeLists |> Seq.cast<Microsoft.CodeAnalysis.SyntaxNode> |> Seq.toList, node.Members |> Seq.cast<Microsoft.CodeAnalysis.SyntaxNode> |> Seq.toList, node.EndOfFileToken)
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.CompilationUnitSyntax as node ->
      Some (node.Options |> Seq.cast<Microsoft.CodeAnalysis.SyntaxNode> |> Seq.toList, node.Imports |> Seq.cast<Microsoft.CodeAnalysis.SyntaxNode> |> Seq.toList, node.Attributes |> Seq.cast<Microsoft.CodeAnalysis.SyntaxNode> |> Seq.toList, node.Members |> Seq.cast<Microsoft.CodeAnalysis.SyntaxNode> |> Seq.toList, node.EndOfFileToken)
    | _ -> None
  ...
  let (|CSharpContinueStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ContinueStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.ContinueKeyword, node.SemicolonToken)
    | _ -> None

  let (|VisualBasicContinueStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|ConversionOperatorDeclaration|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ConversionOperatorDeclarationSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.ImplicitOrExplicitKeyword, node.ExplicitInterfaceSpecifier, node.OperatorKeyword, node.CheckedKeyword, node.Type, node.ParameterList, node.Body, node.ExpressionBody, node.SemicolonToken)
    | _ -> None
  ...
  let (|ConversionOperatorMemberCref|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ConversionOperatorMemberCrefSyntax as node ->
      Some (node.ImplicitOrExplicitKeyword, node.OperatorKeyword, node.CheckedKeyword, node.Type, node.Parameters)
    | _ -> None
  ...
  let (|CrefParameter|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.CrefParameterSyntax as node ->
      Some (node.RefOrOutKeyword, node.RefKindKeyword, node.Type)
    | _ -> None
  ...
  let (|LocalDeclarationStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.LocalDeclarationStatementSyntax as node ->
      Some (node.IsConst, node.AttributeLists |> Seq.toList, node.AwaitKeyword, node.UsingKeyword, node.Declaration, node.SemicolonToken)
    | _ -> None
  ...
  let (|CSharpDoStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.DoStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.DoKeyword, node.Statement, node.WhileKeyword, node.OpenParenToken, node.Condition, node.CloseParenToken, node.SemicolonToken)
    | _ -> None

  let (|VisualBasicDoStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.DoStatementSyntax as node ->
      Some (node.DoKeyword, node.WhileOrUntilClause)
    | _ -> None

  let (|CSharpEmptyStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.EmptyStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.SemicolonToken)
    | _ -> None

  let (|VisualBasicEmptyStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|EnumMemberDeclaration|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.EnumMemberDeclarationSyntax as node ->
      Some (node.AttributeLists |> Seq.cast<Microsoft.CodeAnalysis.SyntaxNode> |> Seq.toList, node.Identifier, node.EqualsValue :> Microsoft.CodeAnalysis.SyntaxNode)
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.EnumMemberDeclarationSyntax as node ->
      Some (node.AttributeLists |> Seq.cast<Microsoft.CodeAnalysis.SyntaxNode> |> Seq.toList, node.Identifier, node.Initializer :> Microsoft.CodeAnalysis.SyntaxNode)
    | _ -> None
  ...
  let (|EventDeclaration|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.EventDeclarationSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.EventKeyword, node.Type, node.ExplicitInterfaceSpecifier, node.Identifier, node.AccessorList, node.SemicolonToken)
    | _ -> None

  let (|CSharpExpressionStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ExpressionStatementSyntax as node ->
      Some (node.AllowsAnyExpression, node.AttributeLists |> Seq.toList, node.Expression, node.SemicolonToken)
    | _ -> None

  let (|VisualBasicExpressionStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|FixedStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.FixedStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.FixedKeyword, node.OpenParenToken, node.Declaration, node.CloseParenToken, node.Statement)
    | _ -> None

  let (|CSharpForEachStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ForEachStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.AwaitKeyword, node.ForEachKeyword, node.OpenParenToken, node.Type, node.Identifier, node.InKeyword, node.Expression, node.CloseParenToken, node.Statement)
    | _ -> None

  let (|VisualBasicForEachStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|ForEachVariableStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ForEachVariableStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.AwaitKeyword, node.ForEachKeyword, node.OpenParenToken, node.Variable, node.InKeyword, node.Expression, node.CloseParenToken, node.Statement)
    | _ -> None

  let (|CSharpForStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ForStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.ForKeyword, node.OpenParenToken, node.Declaration, node.FirstSemicolonToken, node.Condition, node.SecondSemicolonToken, node.CloseParenToken, node.Statement)
    | _ -> None

  let (|VisualBasicForStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|CSharpGenericName|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicGenericName|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|GlobalStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.GlobalStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.Statement)
    | _ -> None
  ...
  let (|GotoStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.GotoStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.GotoKeyword, node.CaseOrDefaultKeyword, node.Expression, node.SemicolonToken)
    | _ -> None
  ...
  let (|CSharpIfStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.IfStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.IfKeyword, node.OpenParenToken, node.Condition, node.CloseParenToken, node.Statement, node.Else)
    | _ -> None

  let (|VisualBasicIfStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|LabeledStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.LabeledStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.Identifier, node.ColonToken, node.Statement)
    | _ -> None
  ...
  let (|LocalFunctionStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.LocalFunctionStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.ReturnType, node.Identifier, node.TypeParameterList, node.ParameterList, node.ConstraintClauses |> Seq.toList, node.Body, node.ExpressionBody, node.SemicolonToken)
    | _ -> None
  ...
  let (|LockStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.LockStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.LockKeyword, node.OpenParenToken, node.Expression, node.CloseParenToken, node.Statement)
    | _ -> None
  ...
  let (|NameColon|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.NameColonSyntax as node ->
      Some (node.Expression, node.Name, node.ColonToken)
    | _ -> None
  ...
  let (|NamespaceDeclaration|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.NamespaceDeclarationSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.NamespaceKeyword, node.Name, node.OpenBraceToken, node.Externs |> Seq.toList, node.Usings |> Seq.toList, node.Members |> Seq.toList, node.CloseBraceToken, node.SemicolonToken)
    | _ -> None
  ...
  let (|OperatorDeclaration|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.OperatorDeclarationSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.ReturnType, node.ExplicitInterfaceSpecifier, node.OperatorKeyword, node.CheckedKeyword, node.OperatorToken, node.ParameterList, node.Body, node.ExpressionBody, node.SemicolonToken)
    | _ -> None
  ...
  let (|OperatorMemberCref|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.OperatorMemberCrefSyntax as node ->
      Some (node.OperatorKeyword, node.CheckedKeyword, node.OperatorToken, node.Parameters)
    | _ -> None
  ...
  let (|CSharpParameter|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicParameter|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|ParenthesizedLambdaExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ParenthesizedLambdaExpressionSyntax as node ->
      Some (node.AsyncKeyword, node.AttributeLists |> Seq.toList, node.ReturnType, node.ParameterList, node.ArrowToken, node.Block, node.ExpressionBody)
    | _ -> None
  ...
  let (|QualifiedName|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.QualifiedNameSyntax as node ->
      Some (node.Left :> Microsoft.CodeAnalysis.SyntaxNode, node.DotToken, node.Right :> Microsoft.CodeAnalysis.SyntaxNode)
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.QualifiedNameSyntax as node ->
      Some (node.Left :> Microsoft.CodeAnalysis.SyntaxNode, node.DotToken, node.Right :> Microsoft.CodeAnalysis.SyntaxNode)
    | _ -> None

  let (|RecordDeclaration|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.RecordDeclarationSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.Keyword, node.ClassOrStructKeyword, node.Identifier, node.TypeParameterList, node.ParameterList, node.BaseList, node.ConstraintClauses |> Seq.toList, node.OpenBraceToken, node.Members |> Seq.toList, node.CloseBraceToken, node.SemicolonToken)
    | _ -> None
  ...
  let (|CSharpReturnStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ReturnStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.ReturnKeyword, node.Expression, node.SemicolonToken)
    | _ -> None

  let (|VisualBasicReturnStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|SimpleLambdaExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.SimpleLambdaExpressionSyntax as node ->
      Some (node.AsyncKeyword, node.AttributeLists |> Seq.toList, node.Parameter, node.ArrowToken, node.Block, node.ExpressionBody)
    | _ -> None
  ...
  let (|Subpattern|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.SubpatternSyntax as node ->
      Some (node.NameColon, node.ExpressionColon, node.Pattern)
    | _ -> None
  ...
  let (|SwitchStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.SwitchStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.SwitchKeyword, node.OpenParenToken, node.Expression, node.CloseParenToken, node.OpenBraceToken, node.Sections |> Seq.toList, node.CloseBraceToken)
    | _ -> None

  let (|CSharpThrowStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ThrowStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.ThrowKeyword, node.Expression, node.SemicolonToken)
    | _ -> None

  let (|VisualBasicThrowStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.ThrowStatementSyntax as node ->
      Some (node.ThrowKeyword, node.Expression)
    | _ -> None

  let (|CSharpTryStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.TryStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.TryKeyword, node.Block, node.Catches |> Seq.toList, node.Finally)
    | _ -> None

  let (|VisualBasicTryStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|UnsafeStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.UnsafeStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.UnsafeKeyword, node.Block)
    | _ -> None
  ...
  let (|UsingDirective|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.UsingDirectiveSyntax as node ->
      Some (node.GlobalKeyword, node.UsingKeyword, node.StaticKeyword, node.Alias, node.Name, node.SemicolonToken)
    | _ -> None

  let (|CSharpUsingStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.UsingStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.AwaitKeyword, node.UsingKeyword, node.OpenParenToken, node.Declaration, node.Expression, node.CloseParenToken, node.Statement)
    | _ -> None

  let (|VisualBasicUsingStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.UsingStatementSyntax as node ->
      Some (node.UsingKeyword, node.Expression)
    | _ -> None

  let (|CSharpWhileStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.WhileStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.WhileKeyword, node.OpenParenToken, node.Condition, node.CloseParenToken, node.Statement)
    | _ -> None

  let (|VisualBasicWhileStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.WhileStatementSyntax as node ->
      Some (node.WhileKeyword, node.Condition)
    | _ -> None

  let (|CSharpYieldStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.YieldStatementSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.YieldKeyword, node.ReturnOrBreakKeyword, node.Expression, node.SemicolonToken)
    | _ -> None

  let (|VisualBasicYieldStatement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.YieldStatementSyntax as node ->
      Some (node.YieldKeyword, node.Expression)
    | _ -> None

  let (|CSharpTypeArgumentList|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.TypeArgumentListSyntax as node ->
      Some (node.LessThanToken, node.GreaterThanToken)
    | _ -> None

  let (|VisualBasicTypeArgumentList|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|ArrayType|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ArrayTypeSyntax as node ->
      Some (node.ElementType :> Microsoft.CodeAnalysis.SyntaxNode, node.RankSpecifiers |> Seq.cast<Microsoft.CodeAnalysis.SyntaxNode> |> Seq.toList)
  ...
  let (|FunctionPointerType|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.FunctionPointerTypeSyntax as node ->
      Some (node.DelegateKeyword, node.AsteriskToken, node.CallingConvention, node.ParameterList)
    | _ -> None

  let (|FunctionPointerParameterList|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.FunctionPointerParameterListSyntax as node ->
      Some (node.LessThanToken, node.GreaterThanToken)
    | _ -> None

  let (|FunctionPointerCallingConvention|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.FunctionPointerCallingConventionSyntax as node ->
      Some (node.ManagedOrUnmanagedKeyword, node.UnmanagedCallingConventionList)
    | _ -> None

  let (|FunctionPointerUnmanagedCallingConventionList|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.FunctionPointerUnmanagedCallingConventionListSyntax as node ->
      Some (node.OpenBracketToken, node.CloseBracketToken)
    | _ -> None

  let (|FunctionPointerUnmanagedCallingConvention|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.FunctionPointerUnmanagedCallingConventionSyntax as node ->
      Some (node.Name)
    | _ -> None
  ...
  let (|NullableType|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.NullableTypeSyntax as node ->
      Some (node.ElementType :> Microsoft.CodeAnalysis.SyntaxNode, node.QuestionToken)
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.NullableTypeSyntax as node ->
      Some (node.ElementType :> Microsoft.CodeAnalysis.SyntaxNode, node.QuestionMarkToken)
    | _ -> None
  ...
  let (|ScopedType|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ScopedTypeSyntax as node ->
      Some (node.ScopedKeyword, node.Type)
    | _ -> None
  ...
  let (|ParenthesizedExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ParenthesizedExpressionSyntax as node ->
      Some (node.OpenParenToken, node.Expression :> Microsoft.CodeAnalysis.SyntaxNode, node.CloseParenToken)
  ...
  let (|AwaitExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.AwaitExpressionSyntax as node ->
      Some (node.AwaitKeyword, node.Expression :> Microsoft.CodeAnalysis.SyntaxNode)
  ...
  let (|MemberAccessExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.MemberAccessExpressionSyntax as node ->
      Some (node.Expression :> Microsoft.CodeAnalysis.SyntaxNode, node.OperatorToken, node.Name :> Microsoft.CodeAnalysis.SyntaxNode)
  ...
  let (|ConditionalAccessExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ConditionalAccessExpressionSyntax as node ->
      Some (node.Expression :> Microsoft.CodeAnalysis.SyntaxNode, node.OperatorToken, node.WhenNotNull :> Microsoft.CodeAnalysis.SyntaxNode)
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.ConditionalAccessExpressionSyntax as node ->
      Some (node.Expression :> Microsoft.CodeAnalysis.SyntaxNode, node.QuestionMarkToken, node.WhenNotNull :> Microsoft.CodeAnalysis.SyntaxNode)
    | _ -> None
  ...
  let (|RangeExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.RangeExpressionSyntax as node ->
      Some (node.LeftOperand, node.OperatorToken, node.RightOperand)
    | _ -> None
  ...
  let (|BinaryExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.BinaryExpressionSyntax as node ->
      Some (node.Left :> Microsoft.CodeAnalysis.SyntaxNode, node.OperatorToken, node.Right :> Microsoft.CodeAnalysis.SyntaxNode)
  ...
  let (|CSharpTypeOfExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.TypeOfExpressionSyntax as node ->
      Some (node.Keyword, node.OpenParenToken, node.Type, node.CloseParenToken)
    | _ -> None

  let (|VisualBasicTypeOfExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|InvocationExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.InvocationExpressionSyntax as node ->
      Some (node.Expression :> Microsoft.CodeAnalysis.SyntaxNode, node.ArgumentList :> Microsoft.CodeAnalysis.SyntaxNode)
  ...
  let (|ExpressionColon|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ExpressionColonSyntax as node ->
      Some (node.Expression, node.ColonToken)
    | _ -> None
  ...
  let (|ImplicitObjectCreationExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ImplicitObjectCreationExpressionSyntax as node ->
      Some (node.NewKeyword, node.ArgumentList, node.Initializer)
    | _ -> None

  let (|CSharpObjectCreationExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicObjectCreationExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|WithExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.WithExpressionSyntax as node ->
      Some (node.Expression, node.WithKeyword, node.Initializer)
    | _ -> None
  ...
  let (|CSharpAnonymousObjectCreationExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicAnonymousObjectCreationExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|CSharpArrayCreationExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicArrayCreationExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|CSharpQueryExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.QueryExpressionSyntax as node ->
      Some (node.FromClause, node.Body)
    | _ -> None

  let (|VisualBasicQueryExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|CSharpFromClause|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicFromClause|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.FromClauseSyntax as node ->
      Some (node.FromKeyword)
    | _ -> None

  let (|CSharpLetClause|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicLetClause|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|WhereClause|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.WhereClauseSyntax as node ->
      Some (node.WhereKeyword, node.Condition :> Microsoft.CodeAnalysis.SyntaxNode)
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.WhereClauseSyntax as node ->
      Some (node.WhereKeyword, node.Condition :> Microsoft.CodeAnalysis.SyntaxNode)
    | _ -> None

  let (|CSharpOrderByClause|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.OrderByClauseSyntax as node ->
      Some (node.OrderByKeyword)
    | _ -> None

  let (|VisualBasicOrderByClause|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|Ordering|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.OrderingSyntax as node ->
      Some (node.Expression :> Microsoft.CodeAnalysis.SyntaxNode, node.AscendingOrDescendingKeyword)
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.OrderingSyntax as node ->
      Some (node.Expression :> Microsoft.CodeAnalysis.SyntaxNode, node.AscendingOrDescendingKeyword)
    | _ -> None

  let (|CSharpSelectClause|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.SelectClauseSyntax as node ->
      Some (node.SelectKeyword, node.Expression)
    | _ -> None

  let (|VisualBasicSelectClause|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|InterpolatedStringExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.InterpolatedStringExpressionSyntax as node ->
      Some (node.StringStartToken, node.Contents |> Seq.cast<Microsoft.CodeAnalysis.SyntaxNode> |> Seq.toList, node.StringEndToken)
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.InterpolatedStringExpressionSyntax as node ->
      Some (node.DollarSignDoubleQuoteToken, node.Contents |> Seq.cast<Microsoft.CodeAnalysis.SyntaxNode> |> Seq.toList, node.DoubleQuoteToken)
    | _ -> None
  ...
  let (|DiscardPattern|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.DiscardPatternSyntax as node ->
  ...
  let (|VarPattern|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.VarPatternSyntax as node ->
      Some (node.VarKeyword, node.Designation)
    | _ -> None

  let (|RecursivePattern|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.RecursivePatternSyntax as node ->
      Some (node.Type, node.PositionalPatternClause, node.PropertyPatternClause, node.Designation)
    | _ -> None

  let (|PositionalPatternClause|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.PositionalPatternClauseSyntax as node ->
      Some (node.OpenParenToken, node.CloseParenToken)
    | _ -> None

  let (|PropertyPatternClause|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.PropertyPatternClauseSyntax as node ->
      Some (node.OpenBraceToken, node.CloseBraceToken)
    | _ -> None
  ...
  let (|ParenthesizedPattern|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ParenthesizedPatternSyntax as node ->
      Some (node.OpenParenToken, node.Pattern, node.CloseParenToken)
    | _ -> None

  let (|RelationalPattern|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.RelationalPatternSyntax as node ->
      Some (node.OperatorToken, node.Expression)
    | _ -> None

  let (|TypePattern|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.TypePatternSyntax as node ->
      Some (node.Type)
    | _ -> None

  let (|BinaryPattern|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.BinaryPatternSyntax as node ->
      Some (node.Left, node.OperatorToken, node.Right)
    | _ -> None

  let (|UnaryPattern|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.UnaryPatternSyntax as node ->
      Some (node.OperatorToken, node.Pattern)
    | _ -> None

  let (|ListPattern|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.ListPatternSyntax as node ->
      Some (node.OpenBracketToken, node.CloseBracketToken, node.Designation)
    | _ -> None

  let (|SlicePattern|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.SlicePatternSyntax as node ->
      Some (node.DotDotToken, node.Pattern)
    | _ -> None
  ...
  let (|Interpolation|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.InterpolationSyntax as node ->
      Some (node.OpenBraceToken, node.Expression :> Microsoft.CodeAnalysis.SyntaxNode, node.AlignmentClause :> Microsoft.CodeAnalysis.SyntaxNode, node.FormatClause :> Microsoft.CodeAnalysis.SyntaxNode, node.CloseBraceToken)
  ...
  let (|InterpolationAlignmentClause|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.InterpolationAlignmentClauseSyntax as node ->
      Some (node.CommaToken, node.Value :> Microsoft.CodeAnalysis.SyntaxNode)
  ...
  let (|CSharpVariableDeclarator|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicVariableDeclarator|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|SwitchExpression|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.SwitchExpressionSyntax as node ->
      Some (node.GoverningExpression, node.SwitchKeyword, node.OpenBraceToken, node.CloseBraceToken)
    | _ -> None

  let (|SwitchExpressionArm|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.SwitchExpressionArmSyntax as node ->
      Some (node.Pattern, node.WhenClause, node.EqualsGreaterThanToken, node.Expression)
    | _ -> None
  ...
  let (|CSharpCatchFilterClause|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicCatchFilterClause|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|FileScopedNamespaceDeclaration|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.FileScopedNamespaceDeclarationSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.NamespaceKeyword, node.Name, node.SemicolonToken, node.Externs |> Seq.toList, node.Usings |> Seq.toList, node.Members |> Seq.toList)
    | _ -> None

  let (|CSharpAttributeList|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicAttributeList|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|CSharpTypeParameterList|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.TypeParameterListSyntax as node ->
      Some (node.LessThanToken, node.GreaterThanToken)
    | _ -> None

  let (|VisualBasicTypeParameterList|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.TypeParameterListSyntax as node ->
      Some (node.OpenParenToken, node.OfKeyword, node.CloseParenToken)
    | _ -> None

  let (|CSharpTypeParameter|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicTypeParameter|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|PrimaryConstructorBaseType|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.PrimaryConstructorBaseTypeSyntax as node ->
      Some (node.Type, node.ArgumentList)
    | _ -> None
  ...
  let (|TypeConstraint|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.TypeConstraintSyntax as node ->
      Some (node.Type :> Microsoft.CodeAnalysis.SyntaxNode)
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.TypeConstraintSyntax as node ->
      Some (node.Type :> Microsoft.CodeAnalysis.SyntaxNode)
    | _ -> None

  let (|DefaultConstraint|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.DefaultConstraintSyntax as node ->
      Some (node.DefaultKeyword)
    | _ -> None

  let (|CSharpFieldDeclaration|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicFieldDeclaration|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|FunctionPointerParameter|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.FunctionPointerParameterSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.Type)
    | _ -> None

  let (|CSharpIncompleteMember|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.IncompleteMemberSyntax as node ->
      Some (node.AttributeLists |> Seq.toList, node.Type)
    | _ -> None

  let (|VisualBasicIncompleteMember|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|CSharpDocumentationCommentTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicDocumentationCommentTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|XmlElement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.XmlElementSyntax as node ->
      Some (node.StartTag :> Microsoft.CodeAnalysis.SyntaxNode, node.Content |> Seq.cast<Microsoft.CodeAnalysis.SyntaxNode> |> Seq.toList, node.EndTag :> Microsoft.CodeAnalysis.SyntaxNode)
  ...
  let (|XmlElementStartTag|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.XmlElementStartTagSyntax as node ->
      Some (node.LessThanToken, node.Name :> Microsoft.CodeAnalysis.SyntaxNode, node.Attributes |> Seq.cast<Microsoft.CodeAnalysis.SyntaxNode> |> Seq.toList, node.GreaterThanToken)
  ...
  let (|XmlElementEndTag|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.XmlElementEndTagSyntax as node ->
      Some (node.LessThanSlashToken, node.Name :> Microsoft.CodeAnalysis.SyntaxNode, node.GreaterThanToken)
  ...
  let (|XmlEmptyElement|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.XmlEmptyElementSyntax as node ->
      Some (node.LessThanToken, node.Name :> Microsoft.CodeAnalysis.SyntaxNode, node.Attributes |> Seq.cast<Microsoft.CodeAnalysis.SyntaxNode> |> Seq.toList, node.SlashGreaterThanToken)
  ...
  let (|XmlName|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.XmlNameSyntax as node ->
      Some (node.Prefix :> Microsoft.CodeAnalysis.SyntaxNode, node.LocalName)
  ...
  let (|XmlCrefAttribute|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.XmlCrefAttributeSyntax as node ->
      Some (node.Name :> Microsoft.CodeAnalysis.SyntaxNode, node.EqualsToken, node.StartQuoteToken, node.Cref :> Microsoft.CodeAnalysis.SyntaxNode, node.EndQuoteToken)
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.XmlCrefAttributeSyntax as node ->
      Some (node.Name :> Microsoft.CodeAnalysis.SyntaxNode, node.EqualsToken, node.StartQuoteToken, node.Reference :> Microsoft.CodeAnalysis.SyntaxNode, node.EndQuoteToken)
    | _ -> None
  ...
  let (|XmlNameAttribute|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    ...
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.XmlNameAttributeSyntax as node ->
      Some (node.Name :> Microsoft.CodeAnalysis.SyntaxNode, node.EqualsToken, node.StartQuoteToken, node.Identifier :> Microsoft.CodeAnalysis.SyntaxNode, node.EndQuoteToken)
  ...
  let (|CSharpXmlProcessingInstruction|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicXmlProcessingInstruction|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|CSharpIfDirectiveTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicIfDirectiveTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|CSharpElseDirectiveTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicElseDirectiveTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.VisualBasic.Syntax.ElseDirectiveTriviaSyntax as node ->
      Some (node.HashToken, node.ElseKeyword)
    | _ -> None

  let (|CSharpEndIfDirectiveTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicEndIfDirectiveTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|CSharpRegionDirectiveTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicRegionDirectiveTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|CSharpEndRegionDirectiveTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicEndRegionDirectiveTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|CSharpBadDirectiveTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicBadDirectiveTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|LineDirectivePosition|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.LineDirectivePositionSyntax as node ->
      Some (node.OpenParenToken, node.Line, node.CommaToken, node.Character, node.CloseParenToken)
    | _ -> None

  let (|LineSpanDirectiveTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.LineSpanDirectiveTriviaSyntax as node ->
      Some (node.HashToken, node.LineKeyword, node.Start, node.MinusToken, node.End, node.CharacterOffset, node.File, node.EndOfDirectiveToken, node.IsActive)
    | _ -> None
  ...
  let (|CSharpReferenceDirectiveTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|VisualBasicReferenceDirectiveTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
  ...
  let (|NullableDirectiveTrivia|_|) (node:Microsoft.CodeAnalysis.SyntaxNode) =
    match node with
    | :? Microsoft.CodeAnalysis.CSharp.Syntax.NullableDirectiveTriviaSyntax as node ->
      Some (node.HashToken, node.NullableKeyword, node.SettingToken, node.TargetToken, node.EndOfDirectiveToken, node.IsActive)
    | _ -> None
